...
namespace newGame
{
    ...
    {
        public static Queue<IDataItem> Items = new Queue<IDataItem>();
        ...
        public void DrawMap(object sender, PaintEventArgs e)
        {
            ...
            for (var i = (int)-Map.LeftUpAngle.Y / MyMap.TileLength; i < MyMap.TilesCount + (int)-Map.LeftUpAngle.Y/ MyMap.TileLength + 1; i++)
            {
                for (var j = (int)-Map.LeftUpAngle.X / MyMap.TileLength; j < MyMap.TilesCount + (int)-Map.LeftUpAngle.X / MyMap.TileLength + 1; j++)
                { 
